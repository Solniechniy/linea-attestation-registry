  ...
  return (
    <div className="w-full flex-col justify-start items-start gap-4 inline-flex">
      <header className="justify-start items-center gap-2 inline-flex">
        ...
        <div className="text-zinc-950 text-base font-semibold">Schema</div>
      </header>
      <div className="w-full flex-col justify-start items-start gap-3 flex">
        <div className="w-full justify-between items-start inline-flex text-gray-700 text-base font-medium">
          <div>{schema.name}</div>
          <div>{cropString(schema.id)}</div>
        </div>
        <div className="text-slate-500 text-sm font-normal leading-tight">{schema.description}</div>
      </div>
      <Link
        to={toSchemaById(schemaId)}
        className="flex gap-2 text-zinc-950 text-sm font-semibold hover:underline items-center"
      >
        View Details
